...
```{r}
Grant_White <- psych %>% dplyr::filter(group == "GRANT") %>% dplyr::select(-c(Case, Sex, Age, group))
```
...
As we can immediately see from the plot, observations 12 and 24 are multivariate ouliers: It is then customary to temporarily remove them and study again the multivariate normality of the dataset.
...
We will not try neither of these improvement, but - for the sake of the exercise - we will proceed in the factor analysis taking this issue into account.
...
Similarly, for variable `V22` we cannot ignore the loading associated to factor 1, besides the higher one being the one of factor 2. Also in this case, we may interpret this result by saying that "problem reasoning" abilities are quite trasversal skills and are surely also related to the verbal sphere.
...
label_GW[!dat$in.ell] = as.character(which(!dat$in.ell))
...
To conclude, we can say that we have found quite a good agreement between the interpretations of the common factors based on the Grant-White students data and on the Pasteur student data. This is reasonable, as we expect that the underlying skills necessary to complete the proposed attitudinal tests were the same - regardless of the school attended by the students. To tell the truth, this concordance among the results was also desirable as it can be used to assess the "stability" of our analysis and interpretation.
...
label_P[!dat$in.ell] = as.character(which(!dat$in.ell))
...
stat_ellipse(level = 0.95, color = "royalblue2", linetype = 2) + 
  stat_ellipse(level = a_P, color = "darkorange") + 
  scale_color_manual(values = c("red", "black")) + 
  geom_text(mapping = aes(x = 0.25, y = 2.7, label = "level = (n-0.5)/n = 0.9965"), 
            colour = "darkorange") +
  geom_text(mapping = aes(x = 0.25, y = 2, label = "level = 0.95"), 
...
From the previous point we have obtained the the first two factors for each school have the same interpretation as "verbal intelligence" and "visual, spatial and logical intelligence". Therefore we expect the plots for the data are very similar and should present the same distribution. The similarity of the plots support even more our analysis.
...
grid.arrange(pl[[1]], pl[[2]], pl[[3]], pl[[4]], pl[[6]], pl[[7]], pl[[8]], pl[[9]], pl[[10]], nrow=3)
```
...
It seems that all these distribution behave well for the most part, but suffer to different extents the effects of lighter-than-expected right tails. In particular, data relative to digits $3$, $6$ and $9$ could be decently modeled by a Gaussian distribution, the same cannot be said the others.\
Now we compare variances. We do not really expect them to be all similar, particularly considered the case of $4$.

To check that the conditional densities all share the same covariance matrix, we compute the Frobenius distances between each class covariance matrix and the pooled covariance matrix, which is ultimately the one used in LDA as estimator of the - allegedly - shared population covariance matrix among classes.
...
We find that there are some digit, namely $5$, $8$, $0$ and $1$, for which the assumption of equal class covariance matrix might be "stretched". We keep this in mind for future reference, as this might be linked to worse performance of the classification of these digits - even more so if we consider that also the normality assumption seemed to be unfitting for these same digits, so we lack any theoretical guarantees on a good performance of LDA.

We should also keep in mind that, even if the assumptions partially fail, the robustness of LDA could guarantee fairly good results regardless.

#### 1. Use linear discriminant analysis (LDA). Display the first two LD variables in a scatterplot, color coding the observations according to variable digit.col below. How well do the discriminate the 10 digits? Refer also to theory.
...
Note that, by default, the priors are the proportions of occurrences of each class in the data set, and they are all pretty similar. The returned means are the centroid coordinates (i.e.: the sample means restricted to the observations of each class) CHECK. COMMENTS ON SCALING AND PROPORTION OF TRACE?
...
This plot gives, by definition of the linear discriminant directions, the "best" bi-dimensional representation of the data for separation purposes. ---- An analysis of the considered scatterplot should therefore give an idea of the overall behavior of the classifier. Note that since the proportion of trace of the first two LDs is just around $0.6$ we do not expect the information provided by the plot to be entirely accurate for a full rank LDA. Indeed, the computations of the misclassification rates of the reduced rank LDA below tell us that keeping only two dimensions does not give accurate results. ---KEEP IT? In the plot we can appreciate that the digit *4* forms a clear and compact cloud at the top, and so does *2* at the left. Meanwhile for the other digits we have some issues as some clouds have some values far away from the centroids (the points highlighted by the black boundaries). The worst case seems to be the digit *5*, which appears to have somewhat of a "bi-modal projected distribution" in this plot, with points concentrated in two separated clouds. Digit *0* and *8* show some overlap with neighboring digits, and *1* even more than them. In general all the other digits do not form a compact distribution around their centroids and display very different variances. \\ We can start making some speculations: the digit *4* and *2* have a clearer separation from the others, with little overlap. Instead, for example the digit *5* could be confused with *9* due to their similar conformations; same for *8* and *0*. \\ The reasons behind this not great outcome could arise from the fact that the assumptions under which the optimal discrimination holds, as previously shown, are not fully met. Another motivation, as we can see in the table above, is that the sum of the trace for first two LD variables is approximately $0.6$, which is a value not particularly high. Further analysis will follow in next points.
...
misclassifications <- data.frame(digit = digits, mis = misclassifications)
p2 <- ggplot(misclassifications, aes(x = digit, y = mis, group = 1)) + 
  geom_point(size = 3) + geom_line(linewidth = 1) + 
  ggtitle("total counts of misclassifications for each digit") + 
  theme(title = element_text(hjust = 0.5))
grid.arrange(p1, p2, nrow = 2)
```

In the confusion matrix we can see the same patterns of the scatterplot: the digit *4* is well discriminated, it has 28 misclassifications and 1116 correct classifications. It is interesting to note that, as previously speculated, the digit *5* has been wrongly predicted as *9* 268 times and the digit *8* and the digit *0* as *8* 77 times. We also remark that the digit *5* is the most difficult to predict with a total of *341* misclassifications, as suggested by the scatterplot. Another interesting feature that arises from the confusion matrix is how some digits are misclassified predominantly in one direction. For example the digit *1* has been misclassified as *2* $206$ times, meanwhile the digit *2* as *1* only $23$ times. The motivation under this behavior is clear in the scatterplot: due to the high variance of the digit *1* a lot of its observations gets close to the centroid of the digit *2* and so they end up being classified as $2$ by the nearest centroid classification. Instead this is not true in the opposite direction. I'M NOT SURE IT'S TRUE ANYMORE
...
Still, the difference of the rates is not very appreciable. This might have to with the fact that leaving a single handwritten digit out has only a marginal impact on the predicition capability of said digit when the model is trained on (around) 249 other digits from the same writer - plus the ones of all other 33 authors.
...
Let's start by observing that $44$ is also the number of different writers who contributed to writing the digits for the training data. If we were under the assumption that the observations are grouped in order of their "author", this $44$-fold cross validation could turn out to be a very effective way of measuring the predictive capability of our model: we would effectively test how well observation from an "unseen" (as in: left out from the training) writer are classified based on the training data of the remaining $43$. However, given the information available on the data-set we cannot be sure that this is the case.\
In the following section we will use two different $44$-fold cross validation error estimates: one that takes a simple average of the error rates over all folds, and one that takes an average weighted on the amount of test data of said folds, to account for the fact that the last group of test data is slightly smaller than the others. Notice that the latter simply amounts to dividing the total number of misclassifications over all the folds and dividing it by the size of the dataset. IS IT TRUE? Since the difference in size of the last fold is of only $8$ observations out of $250$ we do not expect a sensible difference.
...
tibble("rank"=1:9, "arithmetic error"=error44, "weighted error"=error44_w)
```

As expected, the differences are hardly noticeable.

---------- TO BE REMOVED; WHY DIFFERENT? As for the weighted mean:
...
```{r, fig.height=10}
dataf=data.frame("rank" = as.character(9:1), "arithmetic_mean" = error44[9:1], "weighted_mean" = error44_w[9:1])
dataf <- melt(dataf)
...
ggplot(dataf, aes(x=rank, y=value, label = as.character(round(value, 4)), color = variable, group = variable, shape = variable)) + geom_point(size = 3) + 
  ...
  scale_color_manual(values = c("darkorange", "royalblue3"))
```

#### 5. $(optional)$ Find a classification rule that improves on the CV error rate estimates found before. Feel free to use any classification method, even on not covered in class.

Since the assumption of equal covariance matrix across classes seems stretched, out first thought was simply to try Quadratic Discriminant Analysis, which doesn't require it. Note that we wouldn't expect a huge improvement, since it would still rely on the assumption of Gaussian conditional densities.

QDA is, however, impossible to implement in this case. Indeed, QDA requires the inversion of all the covariance matrices in its procedure, not only of the pooled one, and digit 4 doesn't have a full rank covariance matrix (as previously discussed).

```{r, error=FALSE}
qda.fit=qda(digit~.,data=pendigits, CV=TRUE)
qda.err<-1-mean(lda.fitCV$class==pendigits$digit)
qda.err
```

Before switching to another framework, we could try and implemement a similar procedure that ignores the problem of the singularity of the covariance matrix of digit $4$: Regularized Discriminant Analysis. This technique is in between LDA and QDA, as it relies on class covariance matrix estimates that are given by a linear combination of the pooled covariance matrix and of each class sample covariance matrix. The coefficients of the combinations are estimated as to minimize the error rates by default. Quite unexpectedly, the improvements achieved via a 44-fold Cross Validation RDA on the LDA error rates are astounding - where we used again a 44-fold cross validation technique to make error estimates more comparable.
...
Which are around $frac{1}{10}$ of the previous ones.

Since relying less on the assumptions of LDA provides us an improved outcome, the next technique we implement is multinomial logistic regression (a generalisation of the standard binomial logistic regression for categorical variables with more than 2 possible outcomes). Indeed, the strategy is almost identical to the one of LDA (maximization of the discriminant functions), but it relies on the weaker assumptions of logistic regression (the log-odds of each class is assumed to be a linear combination of the explanatory variables).
We repeat k-fold CV on this model using the same criterion above.
...
for(i in 1:k_f){
  ...
  mlog.mis=mlog.mis+sum(!train)-sum(pred.classes == test.data$digit) #number of misclassifications as the total number of test data - the correctly predicted class
}
...
```{r}
print("Arithmetic average of errors: ",mlog.mis)
print("Misclassification rate: ",mlog.err)

```
The results are good, but not as good as those of RDA. Furthermore, the function used to implement this multinomial logistic regression relies on a random choice of initial parameters and the outcome is sensitive to the choice of these initial values (we found that for other seeds the error rates can get really close to those of LDA).

At last, we attempt to use a random forest (with sample sizes as suggested in *The Elements of Statistical Learning*, default), a popular classification method that performs very well on many tasks. Usually the great advantage of random forests is the interpretability of the model, which in this is quite secondary, but it's still worth checking if the errors improve:
*Idea:* forest of classification trees applied to sampled subsets of the training data (with repetition allowed) but with further limitations on the features each tree consider to classify (to guarantee less correlation within the outcomes). Majority vote for the final outcome is used.
...
#  rfor.model = randomForest(digit ~., data = train.data, ntree=100, mtry=sqrt(p))
  ...
  rfor.mis=rfor.mis+sum(pred.classes != test.data$digit) #number of misclassifications as the total number of test data minus the correctly predicted class
}
rfor.err=rfor.err/k_f #error by means of the errors
print("Arithmetic average of errors: ",rfor.err) 

rfor.mis=rfor.mis/n
print("Misclassification rate: ",rfor.mis) 
```
