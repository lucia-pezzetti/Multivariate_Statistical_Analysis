...
pl_sc_GW <- ggplot(dat, aes(x = x, y = y)) + geom_point(aes(col = in.ell), show.legend = F) + geom_text(mapping = aes(label = label),
...
pl_sc_P <- ggplot(dat, aes(x = x, y = y)) + geom_point(aes(col = in.ell), show.legend = F) + geom_text(mapping = aes(label = label),
...
MAYBE NOT SUCH A GOOD IDEA We should also keep in mind that, even if the assumptions partially fail, the robustness of LDA could guarantee fairly good results regardless. In order to have a further check on the multivariate gaussianity, we perform for each digit a statistical test. The null hypothesis is the multivariate gaussianity.

```{r}
result=matrix(data=NA,nrow = 10,ncol=2)
for(i in c(0,1,2,3))
{
  by_digit= pendigits %>% dplyr::filter(digit==i) %>% dplyr::select(-digit)
  mvshapiro_test(as.matrix(by_digit))
  result[i+1,2]=mvshapiro_test(as.matrix(by_digit))$p.value
}
for(i in 0:9){
  result[i+1,1]=i
}
by_digit= pendigits %>% dplyr::filter(digit==4) %>% dplyr::select(-c(y8,digit))
result[5,2]=mvshapiro_test(as.matrix(by_digit))$p.value
...
-Qui vengono p-values praticamente nulli per ogni digit, secondo me non è molto significativo tenere il test-\\
...
In the confusion matrix we can see the same patterns of the scatterplot: the digit *4* is well discriminated, it has 28 misclassifications and 1116 correct classifications. It is interesting to note that, as previously speculated, the digit *5* has been wrongly predicted as *9* 268 times and the digit *8* and the digit *0* as *8* 77 times. We also remark that the digit *5* is the most difficult to predict with a total of $341$ misclassifications, as suggested by the scatterplot. Another interesting feature that arises from the confusion matrix is how some digits are misclassified predominantly in one direction. For example the digit *1* has been misclassifed as *2* $206$ times, meanwhile the digit *2* as *1* only $23$ times. The motivation under this behaviour is clear in the scatterplot: due to the high variance of the digit *1* a lot of its observations gets close to the centroid of the digit *2* and so they end up being classified as $2$ by the nearest centroid classification. Instead this is not true in the opposite direction.

### Use the leave-one-out cross validation (CV). Compute the confusion matrix and the corresponding CV errors. Is it larger than the training error? Why so?
...
#### Compute the $44$-fold cross validation error for each reduced-rank LDA classifier, including full-rank LDA, by using the partition of the observation provided by the variable `groupCV` below. Plot the error curve against the number of discriminant variables. What classifier do you prefer? Comment.
...
```{r, fig.height=10}
dataf=data.frame("rank" = as.character(9:1), "arithmetic_mean" = error44[9:1], "weighted_mean" = error44_b[9:1])
dataf <- melt(dataf)
