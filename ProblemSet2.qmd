...
We again can observe that all the desired assumptions are satisfied. Only two outliers have now been identified, corresponding to observations whose `Case` variable (in the original data set) has value 50 and 155. Globally, this plot seems to agree with the Grant-White one. The only slightly differences are the presence of more outliers in the data from the Grant-White school and a minor horinzontal strecth in the data from the Pasteur one. 
...
tibble("rank"=1:9, "training errors" = train_err44, "arithmetic mean of errors"=error44,
...
for(aux in 1:10){
  ...
  for(i in 1:k_f){
    ...
    test.data=pendigits[!train,]
    test.data$digit=as.factor(test.data$digit)
    knn.pred = class::knn(train=train.data[,1:16], cl=train.data[,17], test=test.data[,1:16], k=aux)
