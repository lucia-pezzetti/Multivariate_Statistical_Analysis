...
In the confusion matrix we can see the same patterns of the scatterplot: the digit *4* is well discriminated, it has 6 misclassifications and 1116 correct classifications. It is interesting to note that, as previously speculated, the digit *5* has been wrongly predicted as *9* 268 times and the digit *8* and the digit *0* as *8* 77 times. We also remark that the digit *5* is the most difficult to predict with a total of $341$ misclassifications, as suggested by the scatterplot. Another interesting feature that arises from the confusion matrix is how some digits are misclassified predominantly in one direction. For example the digit *1* has been misclassifed as *2* $206$ times, meanwhile the digit *2* as *1* only $23$ times. The motivation under this behaviour is clear in the scatterplot: due to the high variance of the digit *1* a lot of its observations gets close to the centroid of the digit *2* and so they end up being classified as $2$ by the nearest centroid classification. Instead this is not true in the opposite direction. 
