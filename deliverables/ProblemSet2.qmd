...
for(i in 1:k_f){
  ...
  #number of misclassifications as the total number of test data - 
  #the correctly predicted class
}
...
We also attempt to use a random forest, a popular classification method that performs very well on many classification tasks. Usually the great advantage of random forests is the interpretability of the model, which in this case is quite secondary, but it's still worth checking if the errors improve. We tried tuning the parameters for the random forest but we did not find significant improvements, so we kept the default ones (`ntree`=500, `mtry`=4).
...
for(i in 1:k_f){
  ...
  rfor.model = randomForest(digit ~., data = train.data)
